// Cricket Arcade — Display (copied from Scrapcore ZERO, lightly adapted).
...
// Browsers only allow fullscreen and an orientation lock from a user gesture,
// so these are called from the first touch rather than at load.
const Fullscreen = {
  tried: false,
  ...
  request() {
    ...
    this.tried = true;
    // Desktop browsers don't need it and it is jarring there.
...
const Display = {
  ...
  init(canvas) {
    ...
    window.addEventListener('orientationchange', () => setTimeout(onChange, 60));
